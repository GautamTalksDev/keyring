...
async function waitForScan(app: ReturnType<typeof createStandaloneApp>, scanId: string) {
  for (let i = 0; i < 120; i++) {
...
async function main() {
  ...
  try {
    ...
    ).cards.filter((c) => c.proposedAction.kind === "revoke" && c.status === "pending");
    ...
    console.log("verification:", (audit.json() as { verification: unknown }).verification);
